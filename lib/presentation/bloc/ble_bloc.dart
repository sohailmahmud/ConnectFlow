...
class BleBloc extends Bloc<BleEvent, BleState> {
  ...
  Future<void> _onStartScanning(
    ...
    _scanSubscription = scanDevicesUseCase(const NoParams()).listen((result) {
      result.fold(
  ...
  Future<void> _onStartDataStream(
    ...
    _dataSubscription = streamDataUseCase(event.deviceId).listen((result) {
      result.fold(
        (failure) =>
            emit(BleError('Data streaming failed: ${failure.toString()}')),
        (data) {
  ...
  Future<void> _onStartFirmwareUpdate(
    StartFirmwareUpdateEvent event,
    Emitter<BleState> emit,
  ) async {
    if (_connectedDevice == null) {
      emit(const BleError('No device connected'));
